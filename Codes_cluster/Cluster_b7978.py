...
# Obtener el F1 score del modelo
...
print("llegue a predicion")
predictions = rf_model.predict(test)
# Predecir las clases para la muestra de test
#predictions = rf_model.predict(test)
...
# Contar la cantidad de objetos clasificados en cada clase
clase1_count = predictions_df[predictions_df['predict'] == 'Variable'].shape[0]
clase2_count = predictions_df[predictions_df['predict'] == 'No_variable'].shape[0]

print(f'Objetos clasificados como clase1: {clase1_count}')
print(f'Objetos clasificados como clase2: {clase2_count}')


#------------------------------------------------------------------------------------

probabilidades_clase_variable = predictions_df['Variable']
probabilidades_clase_no_variable = predictions_df['No_variable']
...
plt.title('Histogram of Probabilities for variables')
plt.xlabel('Probability')
plt.ylabel('Ln(Frequency)')
plt.grid(True)
plt.savefig('variables_proba')
plt.show()
plt.close()
...
plt.yscale('log')  # Establecer el eje Y en escala logarítmica
plt.title('Histogram of Probabilities for No_variables')
plt.xlabel('Probability')
plt.ylabel('Ln(Frequency)')
plt.grid(True)
plt.savefig('no_variables_proba')
plt.show()
plt.close()
